// Font binary header — the 298-byte minimal TrueType font that
// convinces the AMP CDN to cache and serve our content.
// This is REAL font data. The AMP cache sees it and thinks "this is a font",
// then serves the entire response (font + payload) from Google's edge CDN.
//
// Structure:
//   Bytes    0-11:   Offset table (sfVersion, numTables, searchRange, entrySelector, rangeShift)
//   Bytes  12-155:   Table directory (9 entries × 16 bytes)
//   Bytes 156-297:   Minimal table data for cmap, head, hhea, hmtx, maxp, name, post, OS/2, glyf
//
// Total: 298 bytes
...
/** The exact 298-byte font header as a Uint8Array (lazily computed) */
let _fontBytes = null;

/**
 * Returns the 298-byte minimal TrueType font binary.
...
/**
 * Prepends the font header to a Uint8Array payload.
 * Returns a new Uint8Array containing font + payload.
 */
export function prependFontHeader(payload) {
  const font = getFontHeader();
  const result = new Uint8Array(font.length + payload.length);
  result.set(font, 0);
  result.set(payload, font.length);
  return result;
}

/**
 * Strips the font header from an ArrayBuffer or Uint8Array.
 * Returns the remaining bytes.
 */
export function stripFontHeader(buffer) {
  const arr = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);
  if (arr.length < FONT_HEADER_SIZE) {
    throw new Error(
      `Buffer too small to contain font header: ${arr.length} < ${FONT_HEADER_SIZE}`
    );
  }
  // Verify font magic: sfVersion 0x00010000
  if (arr[0] !== 0x00 || arr[1] !== 0x01 || arr[2] !== 0x00 || arr[3] !== 0x00) {
    console.warn(
      "Font magic bytes missing — content may not be font-prepended. " +
      `First 4 bytes: ${Array.from(arr.slice(0, 4)).map(b => b.toString(16).padStart(2, '0')).join(' ')}`
    );
  }
  return arr.slice(FONT_HEADER_SIZE);
}
